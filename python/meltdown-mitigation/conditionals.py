...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    factor = temperature * neutrons_produced_per_second
    stable_lower_end = 0.9 * threshold
    stable_upper_end = 1.1 * threshold

    if factor < stable_lower_end:
        return "LOW"
    if stable_lower_end <= factor <= stable_upper_end:
        return "NORMAL"
